...
shinyServer(function(input, output) {
  ...
  output$my.plot <- renderPlot({
    ...
    ## bounds for opt scen
    upper1.opt <- max(prop.need.prot[1]-sd.prop.need.prot[1],0)
    lower1.opt<- 0
    ...
    upper1.mod <- max(prop.need.prot[2]-sd.prop.need.prot[2],0)
    lower1.mod<- 0
    ...
    upper1.pes <- max(prop.need.prot[3]-sd.prop.need.prot[3],0)
    lower1.pes<- 0
